...
const AdvancedRoutingPopover = React.forwardRef(
    (
        { jsonContext, json, channelIn, channelOut, title, gain, ...props },
        ref
    ) => {
        ...
        const disableRoute = () => {
            ...
            // Other routing. Remove route.
                ...
                json[channelIn.code] = json[channelIn.code].filter(
                    (r) => r.out !== channelOut.code
                );
            }
            jsonContext.reRender();
        };
